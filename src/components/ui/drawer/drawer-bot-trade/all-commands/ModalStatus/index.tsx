              ...
              DATA_SURF?.map((item) => {
                return (
                  <div key={item?.id}
                    ...
                    className={clsx(
                      'w-full min-h-9 rounded-md flex justify-center items-center',

                    )}
                  >
                    <Typography className='text-xs' color={item?.title === value ? '#177DFF' : '#212121'} fontWeight={400}>
                      {item?.title}
                    </Typography>
                  </div>
                )
...
export default forwardRef(ModalStatus);
