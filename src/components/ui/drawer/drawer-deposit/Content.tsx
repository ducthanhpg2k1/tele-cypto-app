...
const Content = () => {
  return (
    <>
      <Image src='/images/qr.png' alt='' width={160} height={160} className='self-center' />
      <Card />
      <Stack className='gap-3 w-full'>
        {tableDeposit.map((e, index) => {
          return <Item key={index} label={e.label} value={e.value} />;
        })}
      ...
      >
        <Typography variant='body2' className='font-medium text-[12px]' color={'#9E9E9E'}>
          {t('deposit.more2')}
        </Typography>
