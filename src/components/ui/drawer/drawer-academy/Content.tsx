...
const Content = () => {
  const refDrawerDetailAcademy: any = useRef();
  return (
    <>
      ...
      <Stack className='gap-3' maxWidth={'calc(100%)'}>
        <Typography variant='h6' className='text-[16px]'>
          {t('academy.bestchoice')}
        </Typography>
        <Stack className='flex-row gap-1.5 overflow-auto pb-1' maxWidth={'calc(100% + 32px)'}>
          {mockDataAirdrop.map((e, index) => {
            return (
              <ItemTopic
                onClick={() => refDrawerDetailAcademy.current.onOpen()}
                img={e.img}
                title={e.title}
                key={index}
              />
      ...
      <DrawerDetailAcademy ref={refDrawerDetailAcademy} />
    </>
  );
};
