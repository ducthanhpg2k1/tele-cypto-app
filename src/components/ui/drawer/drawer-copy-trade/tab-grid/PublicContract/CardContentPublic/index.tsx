      ...
      >
        <div className='flex items-center justify-between'>
          ...
          <div className='flex flex-col items-end gap-[6px]'>
            <TypographyRegular className='text-[10px] leading-3' color={'#757575'}>
              ROI trong 90 ngày
            </TypographyRegular>
            <div className='flex items-center w-max justify-center bg-[#06C14914] py-[2px] px-0.5'>
              <TypographyRegular className='text-[10px] leading-3 text-[#4AAF57] rounded'>
                +6,83%
              </TypographyRegular>
            </div>
          </div>
        ...
        {type === 'deliveredImmediately' ? (
          <div className='flex items-center gap-3'>
            <Button fullWidth variant='blue' className='h-9 font-semibold'>
              <Typography className='text-[12px] font-semibold'>
                {t('copyTrade.filter.simulationBtn')}
              </Typography>
            </Button>
            <Button fullWidth variant='pink' className='h-9 font-semibold'>
              <Typography className='text-[12px] font-semibold'>
                {t('copyTrade.filter.fullBtn')}
              </Typography>
            </Button>
          </div>
        ) : (
          <div className='flex items-center gap-3'>
            <Button fullWidth variant='blue' className='h-9 font-semibold'>
              <Typography className='text-[12px] font-semibold'>{t('copyTrade.reset')}</Typography>
            </Button>
            <Button fullWidth className='h-9 font-semibold'>
              <Typography className='text-[12px] font-semibold'>{t('copyTrade.save')}</Typography>
            </Button>
          </div>
        )}
      </Box>
...
const Information = ({ label, value }: { label: string; value: string }) => {
  return (
    <div className='flex flex-col gap-[2px]'>
      <TypographyRegular className='underline' style={{ color: '#757575' }}>
        {label}
      </TypographyRegular>
